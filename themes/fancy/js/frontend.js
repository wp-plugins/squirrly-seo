...
break;}
return null;};$._setCookie=function(name,value){document.cookie=name+"="+value+"; expires="+(60*24)+"; path=/";};})(jQuery);function abh_loadbox(){abh_loadbox_loaded=true;jQuery(".abh_tabs li").click(function(){jQuery(".abh_tabs li").removeClass('abh_active');jQuery(this).addClass("abh_active");jQuery(".abh_tab").hide();var selected_tab=jQuery(this).find("a").attr("href");jQuery(selected_tab.replace('#','.')+'_tab').fadeIn();jQuery._setCookie('abh_tab',selected_tab);return false;});if(jQuery._getCookie('abh_tab')!==null){jQuery(".abh_tab").hide();jQuery(".abh_tabs li").removeClass('abh_active');var selected_tab=jQuery._getCookie('abh_tab');jQuery(selected_tab.replace('#','.')+'_tab').fadeIn();jQuery(selected_tab).parents('.abh_box').find(selected_tab.replace('#','.')).addClass("abh_active");}}
